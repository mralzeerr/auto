...
async function handleInput(text) {
  ...
  // ---- التالي / أغنية أخرى / غيّر ----
  const wantsNext = /(أخرى|اخرى|اخره|ثانيه|ثانية|غيرها|التالي|التاليه|التالية|اللي بعد|بدل|بدّل|غير|غيّر)/.test(t);
  const mediaNoun = /(اغنية|أغنية|اغنيه|أغنيه|اغاني|أغاني|موسيقى|مقطع|فيديو|فديو)/.test(t);
  if ((wantsNext && mediaNoun) || /^(التالي|التاليه|التالية|اللي بعدها|اللي بعده|عقب|تخطى|تجاوز|سكب|نكست)$/.test(t)) {
    const rm = t.match(/(?:أخرى|اخرى|اخره|ثانيه|ثانية|غيرها)\s+(?:ل|لل)?(.+)$/);
    const artist = rm ? rm[1].trim() : "";
    if (ytPlayer) { playNextTrack(false); return reply("⏭️ حاضر، شغلت لك اللي بعدها.", { speech: "حاضر، اللي بعدها" }); }
    if (mediaMode === "radio" && radioStations.length) { radioTryCount = 0; return playRadioStation(radioIdx + 1); }
    if (artist) return doMusic(artist);
    return doMusic(currentMusicQuery || "اغاني منوعة");
  }

  // ---- الموسيقى ----
  const musicMatch = t.match(/(?:شغل|شغّل|اسمعني|سمعني|ابا اسمع|أبا أسمع|بغيت اسمع|ابي اسمع|ابغى اسمع)\s+(?:لي\s+)?(?:أغنية|اغنية|أغنيه|اغنيه|أغاني|اغاني|موسيقى|نشيد|قرآن|قران|سورة|سوره)?\s*(.*)/);
  ...
  // طلب بدون فعل أمر: «أغنية فلان» / «موسيقى هادية» / «سورة الكهف»
  const nounFirst = t.match(/^(?:ابا|أبا|ابي|ابغى|بغيت)?\s*(?:اغنية|أغنية|اغنيه|أغنيه|اغاني|أغاني|موسيقى|نشيد|سورة|سوره)\s+(.+)$/);
  if (nounFirst) return doMusic(nounFirst[1].trim());
  if (/(وقف|أوقف|اوقف|اقفل|سكر).*(الأغنية|الاغنية|الأغنيه|الاغنيه|الموسيقى|الصوت|الأغاني|الاغاني|الفيديو|الفلم|الفيلم|الأخبار|الاخبار|العرض|المقطع)/.test(t)) return stopMusic();
